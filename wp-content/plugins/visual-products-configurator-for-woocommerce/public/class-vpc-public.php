...
class VPC_Public {
    ...
    public function get_vpc_editor_ajax() {
        ...
        if(isset($_POST["vpc"])){
            $vpc =vpc_array_sanitize( $_POST["vpc"]);
            $to_load = Orion_Library::get_proper_value($vpc, "preload", false);
            if(isset($_POST["vpc"]["product"]))
                $product_id = intval($_POST["vpc"]["product"]);
            $config = vpc_get_product_config($product_id);
            $skin = Orion_Library::get_proper_value($config->settings, "skin", "VPC_Default_Skin");
...
//        var_dump($wp_query->query_vars);
    ...
    public function add_vpc_configuration_to_cart() {
        ...
        if(isset($_POST["recap"])){
            $recap =vpc_array_sanitize($_POST["recap"]);
        }
        if(isset($_POST["custom_vars"])){
            $custom_vars =vpc_array_sanitize($_POST["custom_vars"]);
        }
//        var_dump($custom_vars);

        $alt_products = array();
        if (isset($_POST["alt_products"])){
            $alt_products = vpc_array_sanitize($_POST["alt_products"]);
        }
        if (!is_array($alt_products))
            $alt_products = array();
        if(isset($_POST)){
            $posts=vpc_array_sanitize($_POST);
